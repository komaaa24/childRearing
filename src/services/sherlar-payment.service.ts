...
export class SherlarPaymentService {
    ...
    async hasValidPaymentByTransaction(transactionParam: string): Promise<{ hasPaid: boolean; paymentDate?: Date }> {
        try {
            await this.ensureInitialized();
            ...
            if (result && result.length > 0) {
                const payment = result[0];
                console.log(`✅ Payment found in sherlar DB by transaction: ${transactionParam}`);
                return {
                    hasPaid: true,
                    paymentDate: new Date(payment.created_at)
                };
