...
import { buildScopedSessionKey, resolveBotKey, resolveBotUsername } from "../services/bot-scope.service.js";
...
function resolveSessionKey(userId: number, botUsername: string): string {
    return buildScopedSessionKey(userId, botUsername);
}

async function resolveUserLanguage(ctx: Context, userId: number): Promise<BotLanguage> {
    const botUsername = resolveBotUsername(ctx);
    const detectedLanguage = detectLanguageFromTelegram(ctx.from?.language_code);
    const existingUser = await userService.findByTelegramId(userId, botUsername);
    const resolvedLanguage = normalizeLanguage(existingUser?.preferredLanguage || detectedLanguage);

    await userService.findOrCreate(userId, botUsername, {
...
async function verifyScopedPayment(
    payment: Payment,
    telegramId: number,
    botUsername: string,
    botKey: string
): Promise<"paid" | "pending" | "revoked"> {
    if (payment.status === PaymentStatus.PAID) {
        await botAccessService.markAsPaid(telegramId, botUsername, botKey);
        return "paid";
    }

    const paymentResult = await sherlarPaymentService.hasValidPaymentByTransaction(payment.transactionParam);
    if (!paymentResult.hasPaid) {
        return "pending";
    }

    const access = await botAccessService.getAccess(telegramId, botUsername, botKey);
    ...
    payment.metadata = {
        ...
        verifiedVia: "sherlar_transaction",
        botUsername
    };
...
async function syncScopedAccessFromLatestPayment(
    user: User,
    telegramId: number,
    botUsername: string,
    botKey: string
    ...
    const verificationResult = await verifyScopedPayment(payment, telegramId, botUsername, botKey);
...
export async function handleSetLanguage(ctx: Context, language: BotLanguage) {
    ...
    if (!userId) return;
    const botUsername = resolveBotUsername(ctx);
    const sessionKey = resolveSessionKey(userId, botUsername);
    ...
    await userService.setPreferredLanguage(userId, botUsername, language);
...
async function showJoke(ctx: Context, userId: number, index: number, answerCallback = false) {
    const botUsername = resolveBotUsername(ctx);
    const session = sessions.get(resolveSessionKey(userId, botUsername));
    if (!session) return;
    ...
    const hasPaid = await botAccessService.hasPaidForBot(userId, botUsername, botKey);
...
export async function handleStart(ctx: Context) {
    ...
    const botKey = resolveBotKey(ctx);
    let hasPaid = await botAccessService.hasPaidForBot(userId, botUsername, botKey);
    const sherlarEnabled = (process.env.ENABLE_SHERLAR_CHECK || "false").toLowerCase() === "true";

    if (!hasPaid && sherlarEnabled) {
        console.log(`🔍 [START] Checking scoped payments for user=${userId}, bot=${botUsername}`);
        try {
            hasPaid = await syncScopedAccessFromLatestPayment(user, userId, botUsername, botKey);
        } catch (error) {
            console.error("❌ [START] Scoped payment check error:", error);
        }
    }
...
export async function handleShowJokes(
    ...
    sessions.set(resolveSessionKey(userId, botUsername), {
        jokes,
        currentIndex: 0,
        language
    });
...
export async function handlePayment(ctx: Context) {
    ...
    const alreadyPaid = await botAccessService.hasPaidForBot(userId, botUsername, botKey);
...
export async function handleCheckPayment(ctx: Context, paymentId: number) {
    ...
    const paymentBelongsToCurrentUser =
        payment?.user?.telegramId === userId ||
        Number(payment?.metadata?.telegramId) === userId;

    if (
        !payment ||
        payment.botUsername !== botUsername ||
        !paymentBelongsToCurrentUser ||
        (payment.metadata?.botKey && payment.metadata.botKey !== botKey)
    ) {
        await answerCallbackSafe(ctx, {
    ...
    if (payment.status === PaymentStatus.PAID) {
        await botAccessService.markAsPaid(userId, botUsername, botKey);
        await answerCallbackSafe(ctx, {
    ...
    if (payment.status === PaymentStatus.PENDING) {
        ...
        try {
            ...
            const verificationResult = await verifyScopedPayment(payment, userId, botUsername, botKey);

            if (verificationResult === "paid") {
                await ctx.editMessageText(messages.paymentApprovedText(Number(payment.amount)), {
                    parse_mode: "HTML"
                });
            } else if (verificationResult === "revoked") {
