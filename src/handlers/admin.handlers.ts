...
import { resolveBotKey, resolveBotUsername } from "../services/bot-scope.service.js";
...
export async function handleApproveBytelegramId(ctx: Context, telegramId: number) {
    ...
    const user = await userService.findByTelegramId(telegramId, botUsername);

    if (!user) {
        await ctx.reply(
            `❌ <b>Xatolik!</b>\n\n` +
            `Telegram ID ${telegramId} topilmadi (${botUsername})!\n` +
    ...
    const alreadyPaid = await botAccessService.hasPaidForBot(telegramId, botUsername, botKey);
    ...
    const pendingPayment = await paymentRepo
        ...
        .andWhere("payment.botUsername = :botUsername", { botUsername })
...
export async function handleRevokeByTelegramId(ctx: Context, telegramId: number) {
    ...
    if (!user) {
        return ctx.reply(
            ...
            `Bu ID bilan foydalanuvchi ${botUsername} botida database'da yo'q.`,
            { parse_mode: "HTML" }
        );
    }
    ...
    const botKey = resolveBotKey(ctx);
    const hasPaid = await botAccessService.hasPaidForBot(telegramId, botUsername, botKey);
    ...
    await botAccessService.revoke(telegramId, botUsername, botKey);
