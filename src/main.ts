...
app.post("/internal/send-payment-notification", async (req, res) => {
    try {
        ...
        const scopedBotUsername = normalizeBotUsername(botUsername || bot.botInfo?.username);

        // Send notification via bot with inline button to return to bot
        const { InlineKeyboard } = await import("grammy");
        const keyboard = new InlineKeyboard()
            .url("🔙 Вернуться в бот", `https://t.me/${scopedBotUsername}`);

        const preferredLanguage = await userService.getPreferredLanguage(Number(telegramId), scopedBotUsername);
